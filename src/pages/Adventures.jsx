...
import { assets, adventures, categories, difficulties, durations } from '../assets/assets'
...
function Adventures() {
  ...
  const filteredAdventures = adventures.filter(a => {
    const diffMatch = activeDifficulty === 'All' || a.difficulty === activeDifficulty
    const durMatch = activeDuration === 'All' || a.duration === activeDuration
    return diffMatch && durMatch
  })

  const hasActiveFilters = activeDifficulty !== 'All' || activeDuration !== 'All'
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      {/* ── Filter Bar ── */}
      <div className="sticky top-0 z-30 bg-black border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.7)]">
        <div className="max-w-7xl mx-auto px-4 py-4 space-y-3">

          {/* Row 1: Difficulty */}
          <div className="flex flex-wrap items-center gap-10">
            <span className="text-white/35 text-[11px] font-semibold uppercase tracking-widest w-16 shrink-0">
              Difficulty
            </span>
              ...
              {difficulties.map(diff => {
                ...
                return (
                  <button
                    ...
                    className="px-4 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200 hover:scale-105"
                    style={isActive
                      ? { backgroundColor: '#0A9548', color: 'white', borderColor: '#0A9548', boxShadow: '0 0 12px rgba(10,149,72,0.45)' }
                      : { backgroundColor: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.55)', borderColor: 'rgba(255,255,255,0.15)' }
                    }
                  >
                    {diff}
                  </button>
          ...
          {/* Row 2: Duration + result count + reset */}
          <div className="flex flex-wrap items-center gap-10">
            <span className="text-white/35 text-[11px] font-semibold uppercase tracking-widest w-16 shrink-0">
              Duration
            </span>
            ...
            <span className="text-white/40 text-xs shrink-0">
              <span className="text-white font-bold">{filteredAdventures.length}</span> result{filteredAdventures.length !== 1 ? 's' : ''}
            </span>
            ...
            {hasActiveFilters && (
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium border border-white/20 text-white/50 hover:text-white hover:border-white/40 transition-all duration-200 shrink-0"
              >
        ...
        <div className="max-w-7xl mx-auto">
          {filteredAdventures.length === 0 ? (
            <div className="text-center py-24">
              ...
              <p className="text-white/50 text-lg mb-1">No adventures found</p>
              <p className="text-white/30 text-sm mb-8">Try a different difficulty or duration</p>
              <button
                onClick={resetFilters}
                className="px-6 py-3 rounded-full text-white text-sm font-semibold transition-all hover:scale-105 hover:shadow-[0_0_20px_rgba(10,149,72,0.5)]"
